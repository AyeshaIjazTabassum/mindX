...
from agents.coral_registration_agent import CoralRegistrationAgent
...
class AugmenticSystem:
    ...
    async def initialize(self):
        ...
        # Then in the initialize() method, add after the audit coordinator setup:
        print("🔍 Setting up augmentic audit campaigns...")
        # ... your existing audit code ...

        # Add Coral registration after audit setup
        print("🌐 Registering with Coral Protocol...")
        self.coral_registrar = CoralRegistrationAgent()
        coral_success = await self.coral_registrar.register_all_core_agents(self.coordinator)

        if coral_success > 0:
            print(f"✅ Successfully registered {coral_success} agents with Coral Protocol")
        else:
            print("⚠️  Coral registration completed with some failures")
        ...
        try:
            ...
            self.config.get("mastermind_agent.augmentic_mastermind.tools_registry_path", "data/config/augmentic_tools_registry.json")
            # self.config["mastermind_agent.augmentic_mastermind.tools_registry_path"] = "data/config/augmentic_tools_registry.json"
            ...
            self.token_calculator = TokenCalculatorToolRobust(
                memory_agent=self.memory_agent,
                config=self.config
            )
            ...
            print("🔍 Setting up augmentic audit campaigns...")
            self.audit_coordinator.add_audit_schedule(
